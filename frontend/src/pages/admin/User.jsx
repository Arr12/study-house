...
export default function User({ setLoading, active, setActive }) {
    ...
    dataUser?.data?.data?.map((v, i) => {
        dataFilter?.data?.push({
            number:
                (dataUser?.data?.current_page - 1) * dataUser?.data?.per_page +
                i +
                1,
            username: v?.username,
            nickname: v?.nickname,
            ...
            action: (
                <div className="flex flex-row items-center">
                    ...
                    <div
                        className="bg-gray-500 text-white p-2 mr-3 rounded-lg cursor-pointer"
                        onClick={() => {
                            setModal({ type: "delete", status: true });
                        }}
                    >
                        <Delete />
                    </div>
    ...
    const handleOnSubmit = (event, typeModal) => {
        ...
        var { username, nickname, password, email, bio } = document.forms[0];
        if (typeModal === "edit") {
            putUser(
                username?.value,
                nickname?.value,
                        ...
                        <div className="relative bg-white rounded-lg shadow">
                            <div className="flex items-start justify-between p-5 border-b rounded-t">
                                ...
                                <button
                                    ...
                                    onClick={() =>
                                        setModal({ type: false, status: false })
                                    }
                                >
                                    <svg
                                        className="w-5 h-5"
